...
function redirect_to_home_page()
{
	switch ($_SESSION["accesslevel"]) 
	{
		case "admin":
			...
			break;
		case "user":
			header("Location: user.php?profile_id=". $_SESSION["profileid"]);
			break;
	}
}
...
{
	if (strlen($field) > 200)
		return "Email address must be less than 200 characters<br>";
...
// max length is 255
// Learning PHP, MySQL & JavaScript_ With jQuery, CSS & HTML5 (2014, O’Reilly Media) - Robin Nixon
function validate_employer_name($field)
{
	if (strlen($field) > 500)
		return "Employer name must be less than 500 characters<br>";
	else if (preg_match("/[^a-zA-Z0-9_-&,#@* ]/", $field))
		return "Only a-z, A-Z, 0-9, -, _, *, @, #, &, comma and space are allowed in employer name<br>";
	return "";
}

function validate_employer_type($field)
{
	global $employer_type_enum;
	if (!(in_array($field, $employer_type_enum)))
	{
		return "Invalid employer type";
	}
	return "";
}
?>